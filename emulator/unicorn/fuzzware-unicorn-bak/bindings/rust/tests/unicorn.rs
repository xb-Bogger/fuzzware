...
use std::rc::Rc;
use unicorn::unicorn_const::{uc_error, Arch, HookType, MemType, Mode, Protection, SECOND_SCALE};
...
fn emulate_x86() {
    ...
    assert_eq!(emu.mem_map(0x1000, 0x4000, Protection::ALL), Ok(()));
...
fn x86_code_callback() {
    ...
    let hook = emu
        .add_code_hook(
            unicorn::unicorn_const::HookType::CODE,
            0x1000,
            0x2000,
            callback,
        )
