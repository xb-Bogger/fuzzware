...
use super::{uc_error, Arch, HookType, MemType, Mode, Protection};
...
pub fn add_debug_prints_ARM<D>(uc: &mut super::UnicornHandle<D>, code_start: u64, code_end: u64) {
    ...
    let callback = Box::new(
        move |uc: &mut super::UnicornHandle<D>, addr: u64, size: u32| {
            let sp = uc
    ...
    uc.add_code_hook(HookType::CODE, code_start, code_end, callback)
...
pub fn init_emu_with_heap(
    ...
    unsafe {
        ...
        let arena_ptr = crossplatform_mmap(size as usize);
        uc.mem_map_ptr(
            base_addr,
            size as usize,
...
#[cfg(not(windows))]
unsafe fn crossplatform_mmap(size: usize) -> *mut c_void {
...
unsafe fn crossplatform_mmap(size: usize) -> *mut c_void {
    ...
    // See https://msdn.microsoft.com/en-us/library/windows/desktop/aa366537.aspx
    ...
    let ptr = MapViewOfFile(handle, access, 0, 0, size as SIZE_T);
    CloseHandle(handle);
    ...
    let mut old = 0;
    let result = VirtualProtect(ptr, size as SIZE_T, PAGE_READWRITE, &mut old);
    if result != 0 {
        ptr
    } else {
        std::ptr::null_mut()
