...
def unicorn_trace_code(uc, address, size=0, user_data=None):
    ...
    reg_ids = [
        (73, UC_ARM_REG_R0), (74, UC_ARM_REG_R1), (75, UC_ARM_REG_R2), (76, UC_ARM_REG_R3),
        (77, UC_ARM_REG_R4), (78, UC_ARM_REG_R5), (79, UC_ARM_REG_R6), (80, UC_ARM_REG_R7),
        (81, UC_ARM_REG_R8), (82, UC_ARM_REG_R9), (83, UC_ARM_REG_R10), (84, UC_ARM_REG_R11),
        (85, UC_ARM_REG_R12), (16, UC_ARM_REG_SP), (13, UC_ARM_REG_LR), (14, UC_ARM_REG_PC)
    ]
    reg_vals = []
    for name, reg_id in reg_ids:
        val = uc.reg_read(reg_id)
        reg_vals.append(f"{name}:{hex(val)}")
    reg_str = ';'.join(reg_vals)
    print(f'Hook at {hex(address)} {reg_str}', file=instlist, flush=True)
# add-end
...
def configure_unicorn(args):
    ...
    if args.action_event:
        action = 1
        logger.info("Action event hook enabled")
    else:
        action = 0
    if args.data_event:
        data=1
        logger.info("Data event hook enabled")
    else:
        data=0
    ...
    if args.bintrace_file is not None:
        ...
        bintrace.init_tracing(args.bintrace_file, uc, config, mmio_ranges, data, action)
...
def populate_parser(parser):
    ...
    parser.add_argument('--action-event', action='store_true', default=False, help="Enable action event hook")
    parser.add_argument('--data-event', action='store_true', default=False, help="Enable data event hook")
